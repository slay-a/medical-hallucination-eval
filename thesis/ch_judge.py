...
def blocks(R: Results) -> list:
    ...
    if ftj is not None:
        ...
        b += [P(f"The strongest general model was fine-tuned on MedNLI, 11,232 clinician-written premise and hypothesis pairs from "
                f"MIMIC-III notes (Section 3.4), for {ftj['epochs']} epoch(s) with the {'Adafactor' if ftj.get('optimizer') == 'adafactor' else 'AdamW'} "
                f"optimizer, learning rate {ftj['lr']} and an effective batch size of {ftj['batch']} (Section 4.10). Its MedNLI development accuracy rose to {pct(ftj['dev_accuracy'])} and its test accuracy to "
    ...
    b += [P(f"By the pre-specified rule the judge of the main study is {JUDGE_NAMES.get(best.judge, best.judge)} with "
            f"{MODE_PHRASE.get(best['mode'], best['mode'])} and a support threshold of {best.tau_from_other_subset:.2f}. Its agreement with medical experts is "
