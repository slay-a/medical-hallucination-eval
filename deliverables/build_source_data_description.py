...
def main():
    scripts = [
        ...
        ("mimic_second_judge.py", "Second-judge agreement analysis: scores every base-condition claim with Bespoke-MiniCheck-7B (best mode and threshold from the judge study) and reports per-condition agreement, kappa, confirmed flags and UFR under both or either judge (results/mimic_judge_agreement.csv)."),
        ("audit_sample.py, audit_summary.py", "Manual precision audit of the judge: stratified sample of 320 claims for hand labeling (results_private/, never committed) and the aggregate precision, kappa and corrected unsupported rate (results/audit_summary.csv)."),
    ...
    result_files = [
        ...
        ("mimic_judge_agreement.csv", "Agreement between the selected judge and Bespoke-MiniCheck-7B on the 6,588 base-condition claims, per condition."),
