...
def main():
    scripts = [
        ...
        ("judge_candidates.py", "Judge selection study: five candidate judges (MiniLM, two DeBERTa-v3-large NLI models, MiniCheck DeBERTa and RoBERTa) plus the MedNLI-tuned model, each in two evidence modes (top-3 sentences; whole course in 400-token windows), scored against the 1,781 expert-annotated sentences with thresholds chosen on the other subset; writes results/judge_candidates.csv."),
        ("finetune_mednli.py", "Fine-tunes the strongest general NLI model on MedNLI (2 epochs, lr 1e-5, effective batch 16 as 4 x 4 accumulation, max length 256, embeddings frozen, MPS); saves the best-dev checkpoint outside the repository (~/Desktop/Thesis/models)."),
